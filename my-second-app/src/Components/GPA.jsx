...
function GPA() {
  ...
  const fetchAll = async () => {
    const res = await axios.get("http://localhost:9098/gpas");
    setGpas(res.data);
  };

  const saveGpa = async () => {
    await axios.post("http://localhost:9098/gpa", { gpa });
    fetchAll();
  };

  const searchByGpa = async () => {
    const res = await axios.get(
      `http://localhost:9098/gpa/search?value=${gpa}`
    );
    setGpas(res.data);
  };
  ...
  const filterRange = async () => {
    const res = await axios.get(
      `http://localhost:9098/gpa/range?min=${min}&max=${max}`
    );
    setGpas(res.data);
  };
